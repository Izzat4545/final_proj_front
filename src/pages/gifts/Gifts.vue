...
<template>
  <div class="container mx-auto">
    <div v-if="data.data && data.data.length > 0 && data.data[0].event"
      ...
      <img class="size-[150px] object-cover rounded-md" crossorigin="anonymous" :src="data.data[0].event.image"
        alt="Event image" />
  ...
  <div v-if="!getError" class="flex container mx-auto justify-end gap-2 items-center">
  ...
  <div class="flex justify-center">
    ...
    <div v-if="getError" class="p-4 flex items-center gap-2 mt-5 text-red-800 bg-red-100 rounded-lg">
      <div>{{ getError }}</div>
  ...
  <div class="container mx-auto" v-if="route.params.id && !getError && !loading && data.data.length > 0">
    <div
      ...
      <GiftCard v-for="gift in data.data" v-if="!isList" :gift="gift" :is-public="true" />
    </div>
    <div class="flex-col hidden sm:flex justify-center my-3 gap-5">
      <GiftList v-for="gift in data.data" v-if="isList" :gift="gift" :is-public="true" />
    </div>
    ...
    <div v-if="data.meta.totalGifts > PaginationConfig.PAGE_LIMIT" class="flex mb-3 justify-center">
      <Pagination />
    </div>
  </div>
